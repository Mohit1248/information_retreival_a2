...
# d1 = mango banana mango   (length 3)
# d2 = banana kiwi        (length 2)
# d3 = durian durian        (length 2)
# Collection: mango 2, banana 2, kiwi 1, durian 2 -> 7 tokens.
...
def test_dirichlet_query_likelihood_by_hand(stats):
    ...
    #   d2: (0 + 2)     / (2 + 7) = 2/9
    assert feedback.query_log_likelihood(["mango"], "d1", stats, 7.0) == pytest.approx(math.log(0.4))
    assert feedback.query_log_likelihood(["mango"], "d2", stats, 7.0) == pytest.approx(math.log(2 / 9))
    # two query terms multiply (log-probabilities add)
    expected = math.log(0.4) + math.log((1 + 7 * 2 / 7) / (3 + 7))  # banana in d1: 3/10
    assert feedback.query_log_likelihood(["mango", "banana"], "d1", stats, 7.0) == pytest.approx(expected)


def test_rm1_by_hand(stats):
    # mu = 0 -> P(w|D) is the plain ML estimate.
    #   d1: mango 2/3, banana 1/3      d2: banana 1/2, kiwi 1/2
    # With P(d1|Q)=0.75, P(d2|Q)=0.25:
    #   mango  = .75*2/3               = 0.5
    #   banana = .75*1/3 + .25*1/2     = 0.375
    #   kiwi = .25*1/2               = 0.125
    rm = feedback.estimate_rm1(["d1", "d2"], {"d1": 0.75, "d2": 0.25}, stats, mu=0.0)
    assert rm == pytest.approx({"mango": 0.5, "banana": 0.375, "kiwi": 0.125})
...
def test_rm2_by_hand(stats):
    # Query = [mango], uniform P(D) = 1/2, mu = 0. For each w,
    #   P(w, q) = sum_D P(q|D) P(w|D) P(D)      (the P(w) factors cancel)
    # P(mango|d1) = 2/3 and P(mango|d2) = 0, so only d1 contributes:
    #   mango  = 2/3 * 2/3 * 1/2 = 2/9
    #   banana = 2/3 * 1/3 * 1/2 = 1/9
    #   kiwi = 0   (kiwi is only in d2, where the query term has probability 0)
    # Normalised: mango 2/3, banana 1/3.
    rm = feedback.estimate_rm2(["mango"], ["d1", "d2"], {"d1": 0.5, "d2": 0.5}, stats, mu=0.0)
    assert rm == pytest.approx({"mango": 2 / 3, "banana": 1 / 3})


def test_rm2_uses_all_query_terms(stats):
    # Query = [mango, banana], mu = 0, uniform prior. Both factors must be
    # nonzero: only "banana" survives from d2's side (mango is 0 in d2),
    # so per w:  P(w,Q) = P(w) * prod_i sum_D P(q_i|D) P(D|w).
    #   P(w=mango)  = .5*2/3           = 1/3 ; P(D|mango)  = {d1: 1, d2: 0}
    #       factor(mango q)  = 2/3 ; factor(banana q) = 1/3   -> 1/3 * 2/3 * 1/3 = 2/27
    #   P(w=banana) = .5*1/3 + .5*1/2  = 5/12; P(d1|banana) = (1/6)/(5/12) = 2/5, P(d2|banana) = 3/5
    #       factor(mango q)  = 2/3*2/5 + 0 = 4/15
    #       factor(banana q) = 1/3*2/5 + 1/2*3/5 = 2/15 + 3/10 = 13/30
    #       -> 5/12 * 4/15 * 13/30 = 260/5400 = 13/270
    #   kiwi: mango factor = 0 -> dropped
    rm = feedback.estimate_rm2(["mango", "banana"], ["d1", "d2"], {"d1": 0.5, "d2": 0.5}, stats, mu=0.0)
    a, b = 2 / 27, 13 / 270
    assert rm == pytest.approx({"mango": a / (a + b), "banana": b / (a + b)})


def test_rm3_interpolation_by_hand():
    rm = {"banana": 0.6, "kiwi": 0.4}
    model = feedback.interpolate_rm3(["mango"], rm, lam=0.5, n_terms=10)
    assert model == pytest.approx({"mango": 0.5, "banana": 0.3, "kiwi": 0.2})

    # query terms that are also expansion terms add up; lam = 1 ignores feedback
    model = feedback.interpolate_rm3(["banana", "mango"], rm, lam=0.4, n_terms=10)
    assert model == pytest.approx({"mango": 0.2, "banana": 0.2 + 0.36, "kiwi": 0.24})
    assert feedback.interpolate_rm3(["mango"], rm, lam=1.0, n_terms=10) == pytest.approx(
        {"mango": 1.0, "banana": 0.0, "kiwi": 0.0}
    )
...
def test_query_posterior_weights(stats):
    # scale = |Q| gives P(Q|D) normalised over the set. For Q = [mango], mu = 7:
    # d1: 0.4, d2: 2/9 -> weights 0.4/(0.4+2/9), (2/9)/(0.4+2/9)
    w = feedback.query_posterior_weights(["mango"], ["d1", "d2"], stats, mu=7.0, scale=1.0)
    z = 0.4 + 2 / 9
    assert w == pytest.approx({"d1": 0.4 / z, "d2": (2 / 9) / z})
    # a smaller scale flattens the weights toward uniform
    flat = feedback.query_posterior_weights(["mango"], ["d1", "d2"], stats, mu=7.0, scale=0.01)
    assert abs(flat["d1"] - 0.5) < abs(w["d1"] - 0.5)


def test_rank_by_model_orders_by_cross_entropy(stats):
    # model = all mass on "mango", mu = 7 -> ranks by P(mango|D): d1 (0.4) > d2 (2/9) = d3 (0+2)/(2+7)
    ranked = feedback.rank_by_model({"mango": 1.0}, ["d3", "d2", "d1"], stats, 7.0, k=3)
...
def test_feedback_only_returns_candidate_docs_and_handles_odd_seeds(prepared):
    ...
    out = prepared.relevance_model_feedback("mango", ["d3", "d3", "zzz", "d1"], pool, 10)
    ...
    for seed in ([], ["zzz"]):
        fb = prepared.relevance_model_feedback("mango", seed, pool, 10)
        assert [d for d, _ in fb] == [d for d, _ in prepared.score_candidates("mango", pool, 10)]
...
def test_all_variants_run_without_prior_score_candidates(prepared, monkeypatch, variant):
    ...
    out = prepared.relevance_model_feedback("mango banana", ["d1", "d2"], ["d1", "d2", "d3"], 2)
    assert len(out) == 2
    assert {d for d, _ in out} <= {"d1", "d2", "d3"}
    if variant == "rm3":
        # RM3 keeps the query anchored: "mango" only occurs in d1
        assert out[0][0] == "d1"
    else:
